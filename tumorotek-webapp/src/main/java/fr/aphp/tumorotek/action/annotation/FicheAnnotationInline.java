...
public class FicheAnnotationInline extends FicheAnnotation {
	...
	public void setObj(TKAnnotableObject o) {
		...
		if ((obj.getBanque() != null 
				&& !obj.getBanque().equals(getBankUsedToDrawChamps())) 
												|| isMultipleMode) {
			if (obj.getBanque() != null) {
...
//	}
	 ...
	 */
	public void drawAnnotationPanelContentInline(boolean multi) {
		// efface le panel
		cleanPanel();
...
//				}
				...
				while (champsIt.hasNext()) {
					...
					champRowLayout.appendChild(drawAnnotationInline(champsIt.next(), 
												annoTable,
												multi, ""));
				...
				if (champRowLayout.getChildren().size() == 1) {
	 ...
	 * @return group
	 */
	private Vlayout drawTableAnnotationInline(TableAnnotation table, 
															Integer ordre) {
		Vlayout tableLayout = new Vlayout();
		tableLayout.setId("tableAnnoInline" + ordre);
		tableLayout.setHflex("1");
		Label tableLabel = new Label(table.getNom());
	...
	private HtmlMacroComponent drawAnnotationInline(ChampAnnotation chp, 
								Vlayout annoTable,
								boolean multi,
								String colClass) {
...
//	}
	...
	public void onLaterSwitch() {
		...
		for (int i = 0; i < tables.size(); i++) {
			...
			while (champsIt.hasNext()) {				
				chp = champsIt.next();
				((AnnotationComponent) annoInlineBaseLayout
