...
public class FicheAnnotation extends AbstractFicheController {
	...
	public void setObj(TKAnnotableObject o) {
		...
		if ((obj.getBanque() != null 
				&& !obj.getBanque().equals(bankUsedToDrawChamps)) 
												|| isMultipleMode) {
			if (obj.getBanque() != null) {
	 ...
	 */
	public void drawAnnotationPanelContent(boolean multi, boolean cumulative) {
		...
		if (bankUsedToDrawChamps != null) {
			...
			List<TableAnnotation> tabs = 
				ManagerLocator.getTableAnnotationManager()
					.findByEntiteAndBanqueManager(this.entite, 
														bankUsedToDrawChamps);
			...
			for (int i = 0; i < tabs.size(); i++) {
				...
				while (champsIt.hasNext()) {
					Row newRow = new Row();
					newRow.appendChild(drawAnnotation(champsIt.next(), newRow, 
												annoTable, multi, colClass));
					annoRows.appendChild(newRow);
				}
				collapGroups.add(annoTable);
			}	
			// ouvre la première table si unique
			if (tables.size() == 1 && !cumulative) {
	 ...
	 * @return group
	 */
	private Group drawTableAnnotationGroup(TableAnnotation table, 
															Integer ordre) {
	 ...
	 * @param css class pour definir coloration de l'anno label en 
	 * association avec un catalogue
	 * @return MacorComponent
	 */
	@SuppressWarnings("unchecked")
	private HtmlMacroComponent drawAnnotation(ChampAnnotation chp, 
								Row row, Group annoGroup, boolean multi,
								String colClass) {
		HtmlMacroComponent ua;
		ua = (HtmlMacroComponent)
			page.getComponentDefinition("annoComp", false)
		...
		this.champControllers.add((AnnotationComponent) 
			ua.getFellow("annoDiv").getAttributeOrFellow("annoDiv$composer", true));
	 ...
	 */
	public void switchToStaticOrEditMode(boolean is, boolean show) {
		isStatic = is;
		showButtons = show;
		collapseAllGroups();
	...
	public void onLaterSwitch() {
		...
		for (int i = 0; i < tables.size(); i++) {
			champsIt = ManagerLocator.getTableAnnotationManager()
						.getChampAnnotationsManager(tables.get(i)).iterator();
			while (champsIt.hasNext()) {				
				chp = champsIt.next();
				((AnnotationComponent) annoRows
						.getFellow("chp_" + chp.getChampAnnotationId())
						.getFellow("annoDiv")
						.getAttributeOrFellow("annoDiv$composer", true))
							.switchToStaticOrEditMode(isStatic);
			}
		}
